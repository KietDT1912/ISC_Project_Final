<%-- 
    Document   : checkout
    Created on : Oct 30, 2020, 7:11:49 PM
    Author     : Admin
--%>
                    ...
                    <table class="w-100 table table-striped mt-3">   
                        ...
                        <c:forEach items="${listCart}" var="C" varStatus="track">
                            <tr>
                                ...
                                <td>
                                    <input name="quantity${track.count-1}" value="${C.quantity}" type="number" min="1" max="${C.productQuantity}" class="pl-1" style="width: 60px"/>
                                </td>
                                <td>
                                ...
                                <c:choose>
                                    <c:when test="${sessionScope.account != null}">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="name" class="form-control" placeholder="Enter name" required readonly value="${sessionScope.account.displayName}">
                                            <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small>
                                        </div>
                                            <input type="hidden" name="name1" value="${sessionScope.account.userName}"/>
                                        <div class="form-group">
                                            <label for="sdt">Phone number</label>
                                            <input type="number" name="mobile" class="form-control" placeholder="enter phone number" required readonly value="${sessionScope.account.phone}">
                                        </div>
                                        <div class="form-group">
                                            <label for="address">Address</label>
                                            <textarea class="form-control" rows="3" name="address" required readonly>${sessionScope.account.address}</textarea>
                                        </c:when>
                                    </c:choose>
